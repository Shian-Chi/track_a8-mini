...
"""
from os import stat
from a8mini_PyAPI.siyi_sdk.crc16_python import crc16_str_swap
import logging
from a8mini_PyAPI.siyi_sdk.utils import toHex
...
# --------
@dataclass
class AbsoluteZoomLevelMsg:
    seq:int=0
    success:bool=False

@dataclass
class GimbalAngleMsg:
    seq:int=    0
    yaw:int=    0.0
    pitch:int=  0.0
    roll:int=   0.0
...
class SIYIMESSAGE:
    ...
    def gimbalAngleSetMsg(self, yaw_angle:float, pitch_angle:float):    # [a8mini] yaw:-135~135  pitch:-90~25
        yaw_angle = max(-135.0,min(yaw_angle,135.0))
        pitch_angle = max(-90.0,min(pitch_angle,25.0))

        yaw_data = toHex(int(yaw_angle*10), 16).zfill(4)
        pitch_data = toHex(int(pitch_angle*10), 16).zfill(4)
        yaw_data = yaw_data[2:] + yaw_data[:2]
        pitch_data = pitch_data[2:] + pitch_data[:2]

        # yaw_data = yaw_angle
        # pitch_data = pitch_angle

        data=yaw_data+pitch_data
        cmd_id = COMMAND.GIMBAL_ANGLE_SET
        print(f'yaw:{yaw_data}, pitch:{pitch_data}')

        return self.encodeMsg(data, cmd_id)

    def absoluteZoomLevelMsg(self, int_val, float_val):
        int_val=toHex(int_val, 8)
        float_val=toHex(float_val, 8)
        data = int_val+float_val
        cmd_id=COMMAND.ABSOLUTE_ZOOM_LEVEL
        return self.encodeMsg(data, cmd_id)
